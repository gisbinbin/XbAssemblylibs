package com.xb.xblibrary.RefreshSwipeMenuList.view;
...
public class SwipeMenuLayout extends FrameLayout {
    ...
    private static int MENU_VIEW_ID = 2;
    ...
    private void init() {
        ...
        if (mContentView.getId() < 1) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                CONTENT_VIEW_ID=View.generateViewId();
            }
            mContentView.setId(CONTENT_VIEW_ID);
        }

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            MENU_VIEW_ID=View.generateViewId();
        }
