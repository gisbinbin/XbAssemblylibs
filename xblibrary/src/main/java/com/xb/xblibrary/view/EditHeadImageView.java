...
import com.xb.xblibrary.untils.FileUtil;
import com.xb.xblibrary.untils.Image.ImageClip;
...
public class EditHeadImageView  extends RelativeLayout {
    //裁剪原图
    private ImageView imageView;
    //裁剪框
    private ImageClip clipView;
    ...
    public EditHeadImageView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void init(Context context, AttributeSet attrs) {
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.EditHeadImageView);

        //获取剪切框距离左右的边距, 默认为50dp
        mHorizontalPadding = array.getDimensionPixelSize(R.styleable.EditHeadImageView_mHorizontalPadding,
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 50, getResources().getDisplayMetrics()));
        //获取裁剪框边框宽度，默认1dp
        int clipBorderWidth = array.getDimensionPixelSize(R.styleable.EditHeadImageView_clipBorderWidth,
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1, getResources().getDisplayMetrics()));
        //裁剪框类型(圆或者矩形)
        int clipType = array.getInt(R.styleable.EditHeadImageView_clipType, 1);

        //回收
        array.recycle();
        clipView = new ImageClip(context);
        //设置裁剪框类型
        clipView.setClipType(clipType == 1 ? ImageClip.ClipType.CIRCLE : ImageClip.ClipType.RECTANGLE);
        //设置剪切框边框
