...
import com.xb.xblibrary.RefreshSwipeMenuList.adapter.SwipeMenuAdapter;
...
import com.xb.xblibrary.RefreshSwipeMenuList.interfaces.OnSwipeListener;
import com.xb.xblibrary.RefreshSwipeMenuList.interfaces.OnXScrollListener;
import com.xb.xblibrary.RefreshSwipeMenuList.interfaces.SwipeMenuCreator;
import com.xb.xblibrary.RefreshSwipeMenuList.view.PullToRefreshListFooter;
import com.xb.xblibrary.RefreshSwipeMenuList.view.PullToRefreshListHeader;
import com.xb.xblibrary.RefreshSwipeMenuList.view.SwipeMenuLayout;
import com.xb.xblibrary.RefreshSwipeMenuList.view.SwipeMenuView;

/**
 * Created by binbin1058 on 2018/4/27.
 * listview自定义（下拉刷新、上划加载更多、左滑菜单等）
 */
...
public class RefreshSwipeMenuListView  extends ListView implements AbsListView.OnScrollListener {
    ...
    private boolean mshowlaodmore=true;
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if ((mTouchView == null || !mTouchView.isActive()) && Math.pow(dx, 2) / Math.pow(dy, 2) <= 3) {
                    if (getFirstVisiblePosition() == 0 && (mHeaderView.getVisiableHeight() > 0 || deltaY > 0)) {
                        ...
                        invokeOnScrolling();
                    } else if (((getLastVisiblePosition() == getCount() - 1) && deltaY < 0)) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mEnablePullLoad && mFooterView.getHeight()>0 && mFooterView.getBottomMargin() > PULL_LOAD_MORE_DELTA) {
                    if((getLastVisiblePosition() == getCount() - 1)||PullToRefreshListFooter.mstate==PullToRefreshListFooter.STATE_READY){
                        startLoadMore();
                        resetFooterHeight();
     ...
     * @param enable
     */
    public void setPullLoadEnable(boolean enable,boolean showlaodmore) {
        mEnablePullLoad = enable;
        mshowlaodmore=showlaodmore;
        if (!mEnablePullLoad) {
            mFooterView.hide();
            ...
            mPullLoading = false;
            mFooterView.show();
            mFooterView.setState(PullToRefreshListFooter.STATE_NORMAL,showlaodmore);
    ...
    public void stopLoadMore() {
        if (mPullLoading == true) {
            mPullLoading = false;
            mFooterView.setState(PullToRefreshListFooter.STATE_NORMAL,mshowlaodmore);
        }
        if(!mEnablePullLoad){
            resetHeaderHeight();
            mFooterView.setLable();
        }
    }
    ...
    private void updateFooterHeight(float delta) {
        ...
        if (mEnablePullLoad && !mPullLoading) {
            if (height > PULL_LOAD_MORE_DELTA) { // height enough to invoke load
                // more.
                mFooterView.setState(PullToRefreshListFooter.STATE_READY,mshowlaodmore);
            } else {
                mFooterView.setState(PullToRefreshListFooter.STATE_NORMAL,mshowlaodmore);
            }
        }
    ...
    private void startLoadMore() {
        mPullLoading = true;
        mFooterView.setState(PullToRefreshListFooter.STATE_LOADING,mshowlaodmore);
        if (mListViewListener != null) {
